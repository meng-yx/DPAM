...
if os.path.exists(dataset + "/" + prot  + ".cif"):
    cif = open(dataset + "/" + prot  + ".cif")
    pRd = PdbxReader(cif)
    data = []
    pRd.read(data)
    block = data[0]
    ...
    if pdbx_poly_seq_scheme and entity_poly:
        ...
        if good_entities:
            ...
            residues = {}
            seqs = {}
            rp = open("step1/" + dataset + "/" + prot + ".fa","w")
        ...
        flag = 0
        print ("bad " + prot)
elif os.path.exists(dataset + "/" + prot  + ".pdb"):
    os.system(f'pdb2fasta '+ dataset + "/" + prot + ".pdb > step1/" + dataset + "/" + prot + ".fa")
    with open("step1/" + dataset + "/" + prot + ".fa") as f:
        fa = f.readlines()
    fa[0] = fa[0].split(':')[0] + '\n'
    with open("step1/" + dataset + "/" + prot + ".fa",'w') as f:
        f.write(''.join(fa))
else:
    flag = 0
